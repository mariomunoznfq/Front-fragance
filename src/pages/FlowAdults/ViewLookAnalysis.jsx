...
function ViewLookAnalysis({ onNext, onBack, userData, setUserData }) {
  ...
  const looks = [
    ...
    { id: 4, title: 'URBAN', desc: 'Estilo callejero, oversize y moderno.' },
    {
      id: 5,
      title: 'HIPPIE',
      desc: 'Tejidos naturales, colores y estilo libre.',
    },
    {
      id: 1,
      title: 'CASUAL',
      desc: 'Vaqueros, ropa cómoda para el día a día.',
    },
  ];
  ...
  const handleImageUpload = async (e) => {
    ...
    if (imageFile) {
      if (imageFile.size > 5 * 1024 * 1024) {
        alert(
          'La foto es demasiado pesada. El tamaño máximo permitido es de 5MB.'
        );
        e.target.value = null;
        return;
      }
      ...
      try {
        ...
        alert(
          'Nuestra IA está un poco saturada ahora mismo y no ha podido analizar tu foto. Por favor, selecciona un look manualmente.'
        );
        setSelected(null);
        setImagePreview(null);
        setUserData((prev) => ({
      ...
      if (isKid) {
        ...
        if (userData.plan && !planesPredefinidos.includes(userData.plan)) {
          const planParams = new URLSearchParams({
            descripcion: userData.plan,
          });
          const responsePlan = await fetch(
            `${API_BASE_URL}/generatePlan?${planParams.toString()}`,
            {
              method: 'GET',
              headers: { 'X-API-Key': API_KEY },
            }
        ...
        if (
          userData.attitude &&
          !actitudesPredefinidas.includes(userData.attitude.toLowerCase())
        ) {
          const actitudParams = new URLSearchParams({
            descripcion: userData.attitude,
          });
          const responseActitudText = await fetch(
            `${API_BASE_URL}/generateActitud?${actitudParams.toString()}`,
            {
              method: 'GET',
              headers: { 'X-API-Key': API_KEY },
            }
    ...
    <main className="zara-view-analysis fade-in">
      ...
      >
        {imagePreview ? (
          ...
          >
            ...
            <button
              className="zara-btn-back"
              onClick={() => handleSelect('IA SCAN')}
              style={{
      ...
      <footer className="zara-footer-analysis">
        <button
          className="zara-btn-back"
          onClick={onBack}
          disabled={isLoading || isDetectingLook}
        >
          ATRÁS
        </button>
        <button
