...
function ViewAttitude({ onNext, onBack, userData, setUserData }) {
  const [selected, setSelected] = useState(
    userData.attitude && userData.attitude.length > 20
      ? 'IA TEXT'
      : userData.attitude || null
  );
  ...
  const actitudes = [
    {
      id: 7,
      title: 'IA TEXT',
      desc: 'Escribe cómo te sientes y la IA decidirá.',
    },
  ...
  const handleNext = () => {
    const finalAttitude =
      selected === 'IA TEXT' ? customText : selected.toLowerCase();
  ...
  const isNextDisabled =
    !selected ||
    (selected === 'IA TEXT' &&
      (customText.trim().length < 3 || customText.trim().length > 100));
